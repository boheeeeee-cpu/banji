...
export default function MyDaysScreen() {
  const {
    ...
    dwellMinutes, setDwellMinutes, customPlaces,
  } = useTripStore();
  ...
  // 도착 시간: routeEntries 기준으로 계산 후 entry에 매핑
    ...
    return (
      <ThemedView style={styles.screen}>
        <View style={[styles.header, { paddingTop: insets.top + Spacing.three }]}>
          <ThemedText style={styles.title}>My Days</ThemedText>
        </View>
  ...
  return (
    <ThemedView style={styles.screen}>
      <View style={[styles.header, { paddingTop: insets.top + Spacing.three }]}>
        <ThemedText style={styles.title}>My Days</ThemedText>
        <Pressable onPress={clearMyDays} hitSlop={8}>
          <ThemedText type="small" themeColor="textSecondary">전체 삭제</ThemedText>
        </Pressable>
      </View>
      ...
      <ScrollView contentContainerStyle={[styles.list, { paddingBottom: insets.bottom + BottomTabInset + 80 }]}>
        {departureLocation && (
        ...
        {entries.map((entry, index) => {
          ...
          const showLeg = routeEntries.findIndex(e => e.id === entry.id) > (departureLocation ? -1 : 0) ||
            (departureLocation && routeEntries.findIndex(e => e.id === entry.id) === 0);
      ...
      <View style={[styles.bottomBar, { bottom: insets.bottom + BottomTabInset + Spacing.three }]}>
        <Pressable
          onPress={() => { setMyDaysRouteActive(true); router.navigate('/map'); }}
...
const styles = StyleSheet.create({
  ...
  dwellRow: {
    ...
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingLeft: 24 + Spacing.two + 44 + Spacing.two,
    paddingRight: 24 + Spacing.one,
    paddingBottom: Spacing.two,
  },
  ...
  bottomBar: { position: 'absolute', left: Spacing.four, right: Spacing.four },
